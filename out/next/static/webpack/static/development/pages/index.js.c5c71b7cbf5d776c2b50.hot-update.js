    ...
    value: function play(i, j) {
      if (i !== 0 && j !== 0 && i !== 12 && j !== 12 && !(this.state.arrayEl[i] && this.state.arrayEl[i][j])) {
        this.setState(function (prev) {
          ...
          for (var x = 0; x < 12; x++) {
            if (prev.arrayEl[x] && prev.arrayEl[x][y] == prev.currentPlayer) while (queue.length !== 0) {
              var point = queue.pop();

              for (var _x = point.x - 1; _x <= point.x + 1; _x++) {
                for (var _y = point.y - 1; _y <= point.y + 1; _y++) {
                  if (prev.arrayEl[_x] && prev.arrayEl[_x][_y] == prev.currentPlayer && (!arrayVis[_x] || !arrayVis[_x][_y])) {
                    queue.push({
                      x: _x,
                      y: _y
                    });
...
//# sourceMappingURL=index.js.c5c71b7cbf5d776c2b50.hot-update.js.map